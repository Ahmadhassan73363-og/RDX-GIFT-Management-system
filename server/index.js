  ...
  try {
    const result = await pool.query(
      ...
      [
        ...
        reqData.trackingNumber || `GFT-2026-${Math.floor(1000 + Math.random() * 9000)}`,
...
// START SERVER
app.listen(PORT, () => {
  console.log(`🚀 RDX Gift Management Backend running on http://localhost:${PORT}`);
  console.log(`📊 Connected to PostgreSQL (database: ${process.env.PGDATABASE || 'rdx_gift_db'})`);
});
